...
module.exports = {
  ...
  /*getBudgetHistoryDetailById*/
  async getBudgetHistoryDetailById(req, res) {
    try {
      const budgetId = req.query.id;

      const budgetHistory =
        await budgetBooksServices.findBudgetHistoryDetailById(budgetId);

      if (!budgetHistory) {
        return res
          .status(404)
          .json({ success: false, message: "Budget history not found" });
      }
      ...
      console.error("error in budgetHistoryDetailById:", error);
      return res.status(500).json(commonHelper.errorResponse(error.message));
    }
  },
};
