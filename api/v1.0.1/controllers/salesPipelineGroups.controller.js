...
module.exports = {
  ...
  async getSalesPipelinesGroupsById(req, res) {
    try {
      const pipelinesGroupsId = parseInt(req.query.id, 10);
      if (!pipelinesGroupsId || isNaN(pipelinesGroupsId)) {
        return res.status(400).json({
          status: false,
          message: "Sales Pipelines Groups ID is required and must be a number",
        });
      }
      ...
      logger.error(commonHelper.customizeCatchMsg(err));
      res.status(500).json({
        status: false,
        message: "Failed to fetch Sales Pipelines Groups",
        error: err.message,
  ...
  async updateSalesPipelinesGroups(req, res) {
    try {
      ...
      const pipelinesGroups =
        await salesPipelineGroupsServices.updateSalesPipelinesGroups(
          pipelinesGroupsId,
          updatedData
        );
  ...
  async deleteSalesPipelinesGroups(req, res) {
    try {
      const id = req.query.id;

      const deleted =
        await salesPipelineGroupsServices.deleteSalesPipelinesGroups(id);
